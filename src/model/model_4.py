...
class Gft2dBlock(nn.Module):
  ...
  def forward(self, x, EigVec0, EigVec1):
    x = x.reshape(9, -1).transpose(1,0)
    F_hat = calc_2D_graph_fourier_torch(x, torch.from_numpy(EigVec0).to(device).float(), torch.from_numpy(EigVec1).to(device).float())
    if self.rdc!= 1:
    ...
    F_re = calc_inv_2D_graph_fourier_torch(
             (conv).reshape(n_rdc, 9), torch.from_numpy(EigVec0[:, :n_rdc]).to(device).float(),
                                    torch.from_numpy(EigVec1).to(device).float())
...
class AI_Decoder(nn.Module):
    def __init__(self, latent_dim, layer_dim, output_dim):
        super().__init__()
...
class model_4(model):
    def __init__(self,
                 ...
                 nfeat=0, nhid=0, nout=0, nveh=0, depth=0, 
                 weighted=False, rdc=1, bias=True, dropout=0, pred_frames=125) -> None:
...
# def generate_model(idx=4, nfeat= 75, nhid=50, nout=3, nveh=9, depth=1, weighted=False,
#                    rdc=1, bias=True, dropout= 0, pred_frames = 125):
#     model = model_4(idx, nfeat, nhid, nout, nveh, depth, weighted,
#                    rdc, bias, dropout, pred_frames)
#     return model

def generate_model(**model_params):
    model = model_4(**model_params)
    return model
