...
class EncoderFull(nn.Module):
    def __init__(self, *, encoderI_type=encoderI_types[0], encoderT_type=encoderT_types[0], merge_type=merge_types[0], encoderI_args=None, encoderT_args=None, merge_args=None,z_dim_t=32,z_dim_i=64,z_dim_m=64,image_size=200,channels=1,traj_size=3,nmb_prototypes_infra=512,nmb_prototypes_merge=512,normalize_z_infra = True,normalize_z_merge = True,):
        super().__init__()
    ...
    def forward(self,
        ...
        if self.normalize_z_infra:
            z_image = nn.functional.normalize(z_image, dim=1, p=2)
        packed_trajectory = torch.nn.utils.rnn.pack_padded_sequence(x_traj[0], x_traj[1], batch_first=True, enforce_sorted=False)
        ...
        if self.normalize_z_merge:
            z_merged = nn.functional.normalize(z_merged, dim=1, p=2)
        ...
        if return_merge_cXz:
            #TODO
            Output['cXz_merge'] = self.merge_prototypes(z_merged)
        if return_merge_q:
            #TODO
...
class model_100(model):
    def __init__(self, 
        ...
        if self.image_size == 400:
            ...
            #self.layerDec5 = nn.ConvTranspose2d(64, 2,6,1)
            self.layerDec1 = nn.ConvTranspose2d(self.z_dim_m, 2,6,2)
            self.layerDec2 = nn.ConvTranspose2d(2, 2,8,2)
            self.layerDec3 = nn.ConvTranspose2d(2, 2,8,2)
            self.layerDec4 = nn.ConvTranspose2d(2, 2,8,2)
            self.layerDec5 = nn.ConvTranspose2d(2, 2,6,1)
            self.layerDec6 = nn.ConvTranspose2d(2, 2,6,1)
    ...
    def forward(self,
                ...
                x_traj,
                return_infra_z=False, return_infra_cXz=False, return_infra_q=False,
                return_merge_cXz=False, return_merge_q=False,no_recon=False):
