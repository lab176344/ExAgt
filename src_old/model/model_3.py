...
class model_3(model):
    def __init__(
        ...
        # projection head
        if block == BasicBlock:
            sizes = [512] + projector_dim
        else:
            sizes = [2048] + projector_dim
        layers = []
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1):
        ...
        for i in range(1, blocks):
            layers.append(
                block(self.in_planes, planes, downsample=nn.Sequential()))

        return nn.Sequential(*layers)

    def forward(self, x):
        representations = self._forward_backbone(x)
        embedding = self._forward_projector(representations)
        return embedding, representations
