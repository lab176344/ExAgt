...
class loss_101(loss):
    ...
    def forward(self,
        ...
        batch_infra = torch.stack((data['cXz_infra_a'],data['cXz_infra_pp'],data['cXz_infra_pn']))
        batch_merge = torch.stack((data['cXz_merge_a'],data['cXz_merge_pp']))

        # SWAV
        loss_infra_swav = self.swav_loss(batch_infra)
        loss_merge_swav = self.swav_loss(batch_merge)
